import zillowPlugin from "../src/plugins/sources/zillow"
import type { SearchCriteria } from "../src/models/search-criteria"

async function testZillow() {
  console.log("Testing Zillow plugin...\n")
  ...
  const properties = []
  let count = 0
  const maxProperties = 100 // Increased to test more pages

  console.log("Fetching properties...")

  try {
    for await (const property of zillowPlugin.search(criteria, {
    ...
    if (invalidPrice === 0 && invalidAcres === 0) {
      ...
      console.log(`   Server-side filtering needs investigation`)
    }
    console.log("=".repeat(70))
